export class Berenjena {
    ...
    spawnInArea(cantidad = 5, xMin = 0, xMax = 800, yMin = 0, yMax = 600) {
      ...
      for (let i = 0; i < cantidad; i++) {
        ...
        const berenjena = this.group.create(worldX, worldY, 'berenjena');
        berenjena.body.setAllowGravity(false);
      }
    }
    ...
    spawnAtPositions(positions = []) {
      ...
      positions.forEach(({ x, y }) => {
        const berenjena = this.group.create(x, y, 'berenjena');
        berenjena.body.setAllowGravity(false);
      });
    }
  
    /**
     * Callback de recolección por overlap: desactiva berenjena y suma puntos
     */
    collect(player, berenjena) {
      berenjena.disableBody(true, true);
